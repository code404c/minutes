...
def test_prepare_job_normalizes_media_and_dispatches_transcription(service_env: ServiceEnv, monkeypatch) -> None:
    e = service_env
...
def test_end_to_end_pipeline_completes_with_fake_inference(service_env: ServiceEnv, monkeypatch) -> None:
    e = service_env
    job_id = create_test_job(e)
...
def test_prepare_job_marks_failure_when_media_processing_errors(service_env: ServiceEnv, monkeypatch) -> None:
    e = service_env
    broken_path = e.tmp_path / "broken.m4a"
    broken_path.write_bytes(b"broken")
    job_id = create_test_job(e, media_path=broken_path)

    def raise_media_error(_path):
...
def test_pipeline_stage_reentry_after_completion_is_noop(service_env: ServiceEnv, monkeypatch) -> None:
    e = service_env
    job_id = create_test_job(e)
