...
def test_prepare_job_nonexistent_job_is_noop(service_env) -> None:
...
def test_prepare_job_skips_completed_job(service_env) -> None:
...
def test_prepare_job_sync_mode_duration_limit_exceeded(service_env, monkeypatch) -> None:
    """同步模式下超过时长限制应标记 FAILED。"""
    e = service_env
    job_id = create_test_job(e["session_factory"], e["tmp_path"], e["media_path"], sync_mode=True)
...
def test_prepare_job_reentry_with_existing_normalized_path(service_env) -> None:
...
def test_finalize_job_nonexistent_job_is_noop(service_env) -> None:
...
def test_finalize_job_missing_raw_transcript(service_env) -> None:
...
def test_finalize_job_invalid_json_marks_failed(service_env) -> None:
...
def test_finalize_job_skips_when_result_already_exists(service_env) -> None:
...
def test_finalize_job_skips_queued_status(service_env) -> None:
...
def test_transcribe_job_nonexistent_job_is_noop(service_env) -> None:
...
def test_transcribe_job_skips_completed_job(service_env) -> None:
...
def test_transcribe_job_skips_when_normalized_path_missing(service_env) -> None:
...
def test_transcribe_job_skips_when_raw_transcript_exists(service_env) -> None:
...
def test_transcribe_job_fake_inference_produces_result(service_env) -> None:
...
def test_mark_retry_exhausted_nonexistent_job_is_noop(service_env) -> None:
    """不存在的 job 应静默返回。"""
    e = service_env
    service = InferenceService(
        settings=e["settings"],
...
def test_mark_retry_exhausted_skips_already_completed(service_env) -> None:
...
def test_mark_retry_exhausted_without_max_retries(service_env) -> None:
